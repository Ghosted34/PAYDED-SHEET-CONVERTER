...
function deduplicate(rows) {
    ...
    for (const row of rows) {
        ...
        if (!seen.has(sig)) { seen.add(sig); cleaned.push(row); }
...
function tryDelete(filePath) {
    try { if (filePath && existsSync(filePath)) unlinkSync(filePath); }
    catch (e) { console.warn("[cleanup] Could not delete temp file:", e.message); }
}
...
export const batchUpload = async (req, res) => {
    ...
    try {
        ...
        if (!rawData.length) return res.status(400).json({ error: "File is empty or invalid" });
    ...
    `);
        ...
        for (const [payclass, rows] of payclassMap.entries()) {
            ...
            if (!db) {
                console.warn(`[adjustments] No DB mapping for payclass ${payclass}, skipping`);
                continue;
            }

            const bpDescriptions = [...new Set(rows.map((r) => r.bp?.toLowerCase().trim()).filter(Boolean))];
        ...
        for (const [sheetName, records] of Object.entries(recordsBySheet)) {
            utils.book_append_sheet(outWorkbook, utils.json_to_sheet(records), sheetName);
        }
...
export const template = (req, res) => {
    ...
    res.setHeader("Content-Disposition", "attachment; filename=payment-adjustments_template.xlsx");
